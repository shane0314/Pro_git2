...
int main(int argc,char *argv[])
{
     ...
     while(1)
    {
        ...
        //直接获取了事件数量，给出了活动的流，这里就是跟select，poll区别的关键 //select要用遍历整个数组才知道是那个文件描述符有事件。而epoll直接就把有事件的文件描述符按顺序保存在eventList中
        for(int i=0;i<ret;i++)
        {
            //错误输出
...
void Handle(int clientfd)
{
    ...
    //各种处理
    printf("接收到的数据:%s \n",recvBuf);
    Upper(recvBuf,recvLen);
    sendLen = send(clientfd,recvBuf,recvLen,0);
    if( sendLen < 0 )
        perror("send error");
    return ;
}
